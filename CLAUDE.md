...
## เพดานจำนวน client ของ SPP

มีสองชั้น: เพดานของ Bluedroid (`BTA_JV_MAX_RFC_SR_SESSION` ปกติ 3) กับ `SPP_MAX_CLIENTS`
ของเรา พอชนเพดาน Bluedroid มันปฏิเสธที่ชั้น RFCOMM เลย **ไม่มี event ส่งมาถึงโค้ด
และไม่มี log** ดังนั้นตัวเตะออก (newest-wins) จะทำงานเฉพาะเมื่อโต๊ะของเราเต็มก่อน
คือต้องตั้ง `SPP_MAX_CLIENTS` ต่ำกว่าเพดาน stack

อย่าไปสรุปว่าเตะออกไม่ทำงานถ้ายังไม่ได้วัดเพดานจริงก่อน — ต่อทีละเครื่องแล้วดู
`clients spp=` ว่าหยุดที่เท่าไหร่
